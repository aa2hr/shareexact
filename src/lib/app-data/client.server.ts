...
function tokenIdentityKey(token: string): string {
  ...
  if (payload) {
    try {
      ...
      // A malformed token is expected input, not an error condition: fall
      // through to hashing the raw token so the caller still gets a stable
      // cache key instead of an exception.
    }
  }
  return createHash("sha256").update(token).digest("base64url");
}
